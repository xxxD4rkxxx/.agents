...
- The task is unrelated to agent orquestrador
...
```bash
python agent-orquestrador/scripts/scan_registry.py
```
...
```bash
python agent-orquestrador/scripts/match_skills.py "<solicitacao do usuario>"
```
...
```bash
python agent-orquestrador/scripts/orchestrate.py --skills skill1,skill2 --query "<solicitacao>"
```
...
```bash
python agent-orquestrador/scripts/scan_registry.py && python agent-orquestrador/scripts/match_skills.py "<solicitacao>"
```
...
O registry vive em:
```
agent-orquestrador/data/registry.json
```
...
python agent-orquestrador/scripts/scan_registry.py --status

## Re-Scan Completo (Ignora Cache)

python agent-orquestrador/scripts/scan_registry.py --force
```
...
```bash
python agent-orquestrador/scripts/match_skills.py --project meu-projeto "query aqui"
```
...
Quando multiplos skills sao relevantes, o orquestrador classifica o padrao:
...
```
agent-orquestrador/data/projects.json
```
