#!/usr/bin/env python3
"""
Auto-Discovery Engine for Agent orquestrador.
...
def find_skill_files() -> list[Path]:
    ...
    for base in SEARCH_PATHS:
        ...
        for root, dirs, files in os.walk(base):
            ...
            # Skip the orquestrador itself
            if "agent-orquestrador" in Path(root).parts:
                continue
...
def extract_capabilities(description: str) -> list[str]:
    ...
    desc_words = set(re.findall(r'[a-zA-ZÃ€-Ã¿]+', desc_lower))
    caps = []
...
def print_status(registry: dict):
    ...
    print(f"\n{'='*80}")
    print(f"  Agent orquestrador - Skill Registry Status")
