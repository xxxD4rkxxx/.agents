...
```typescript
// Chrome DevTools depurador
function processOrder(order: Order) {
    depurador;  // Execution pauses here
    ...
    if (order.items.length > 10) {
        depurador;  // Only breaks if condition true
    }
...
**VS Code depurador Configuration:**
```json
// .vscode/launch.json
{
    "version": "0.2.0",
...
```python
# Built-in depurador (pdb)
import pdb

def calculate_total(items):
    total = 0
    pdb.set_trace()  # depurador starts here
...
- **Not Using depurador**: console.log isn't always best
