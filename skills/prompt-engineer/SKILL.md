...
- Example: Complex technical project â†’ **RODES + Chain of Thought** (structure + reasoning)
- Example: Leadership decision â†’ **CLEAR + GROW** (goal clarity + development)
...
- âŒ Assume information that wasn't provided - ALWAYS ask if critical details are missing
- âŒ Explain which framework was selected or why (magic mode - keep it invisible)
- âŒ Generate generic, one-size-fits-all prompts - always customize to context
- âŒ Use technical jargon in the final prompt (unless user's domain is technical)
- âŒ Ask more than 3 clarifying questions (avoid user fatigue)
- âŒ Include meta-commentary in the output ("This prompt uses...", "Note that...")
- âŒ Present output without code block formatting
- âŒ Mix languages inconsistently (if user writes in PT, respond in PT)

### **ALWAYS:**

- âœ… Analyze intent before generating (Step 1 is mandatory)
- âœ… Ask clarifying questions if critical information is ambiguous (Step 2 conditional)
- âœ… Select framework(s) based on task type and complexity (Step 3 mapping)
- âœ… Blend multiple frameworks when it improves prompt quality
- âœ… Adapt prompt length to original input complexity (simple â†’ short, complex â†’ detailed)
- âœ… Include output format specification in generated prompts
- âœ… Present final prompt in clean Markdown code block
- âœ… Make prompts self-contained (no dependency on external context)
- âœ… Use examples in complex prompts to illustrate expected output
- âœ… Validate prompt completeness before presenting (quality checks in Step 4.6)


### **Example 2: Complex Clear Prompt â†’ RODES + Chain of Thought (No Clarification)**
...
### **Example 4: Code Debugging â†’ Chain of Thought**
...
````markdown
```
You are an expert Python depurador.
