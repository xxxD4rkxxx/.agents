...
Review the entire codebase file by file. Read the code carefully. Fix issues right away. Don't just note problemsâ€”make the necessary changes.  
...
Check if the `.gitignore` file exists and is thorough. If itâ€™s missing or not complete, update it to include all junk file patterns above. Ensure that `.env.example` exists with keys but no values.  
...
- Duplicate logic (same code in multiple placesâ€”combine)  

**Code Quality (fix issues as you go):**  
- Vague names: `data`, `info`, `temp`, `thing` â†’ rename to be descriptive  
- Magic numbers: `if (status === 3)` â†’ extract to named constant  
- Debug statements: remove `console.log`, `print()`, `depurador`  
...
- SQL: No string concatenation in queriesâ€”use parameterized queries only  
...
**Database:**  
- N+1 queries: loops with database calls inside â†’ use JOINs or batch queries  
- Missing indexes on WHERE/ORDER BY columns  
- Unbounded queries: add LIMIT or pagination  
- Avoid `SELECT *`: specify columns  

**API Design:**  
- Heavy operations (like email, reports, file processing) â†’ move to a background queue  
...
**Reusability:**  
- Duplicate code â†’ extract to shared utilities  
...
[src/api/users.js]  
  ...
  âœ‚ Removed dead function: formatOldWay()  
  ðŸ”§ Renamed 'data' â†’ 'userData' for clarity  
  ðŸ›¡ Added try/catch around API call (line 47)  

[src/db/queries.js]  
  âš¡ Fixed N+1 query: now uses JOIN instead of loop  

SECURITY ISSUES:  
ðŸš¨ CRITICAL: Hardcoded API key in config.js (line 12) â†’ moved to .env  
âš ï¸ HIGH: SQL injection risk in search.js (line 34) â†’ fixed with parameterized query  

SCALABILITY:  
âš¡ Added pagination to /api/users endpoint  
âš¡ Added index on users.email column  

FINAL STATUS:  
âœ… CLEAN - Ready to push to GitHub  
...
- Fix issues immediately, donâ€™t just document them  
- If uncertain about removing something, ask the user  
- Test after making changes  
- Be thorough but practicalâ€”focus on real problems  
- Security issues are blockersâ€”nothing should ship with critical vulnerabilities  

## Related Skills  

- `@auditor-seguranca` - Deeper security review  
