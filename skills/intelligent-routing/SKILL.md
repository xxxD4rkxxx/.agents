...
graph TD
    ...
    D --> F
    E --> F
    F --> G[auditor-seguranca + backend-specialist]
...
| ------------------- | ------------------------------------------ | ------------------------------------------- | ------------ |
| **Authentication**  | "login", "auth", "signup", "password"      | `auditor-seguranca` + `backend-specialist`   | âœ… YES       |
| **UI Component**    | "button", "card", "layout", "style"        | `frontend-specialist`                       | âœ… YES       |
| **Mobile UI**       | "screen", "navigation", "touch", "gesture" | `desenvolvedor-mobile`                          | âœ… YES       |
...
| **Bug Fix**         | "error", "bug", "not working", "broken"    | `depurador`                                  | âœ… YES       |
| **Test**            | "test", "coverage", "unit", "e2e"          | `test-engineer`                             | âœ… YES       |
| **Deployment**      | "deploy", "production", "CI/CD", "docker"  | `devops-engineer`                           | âœ… YES       |
| **Security Review** | "security", "vulnerability", "exploit"     | `auditor-seguranca` + `penetration-tester`   | âœ… YES       |
| **Performance**     | "slow", "optimize", "performance", "speed" | `otimizador-desempenho`                     | âœ… YES       |
| **Product Def**     | "requirements", "user story", "backlog", "MVP" | `product-owner`                             | âœ… YES       |
| **New Feature**     | "build", "create", "implement", "new app"  | `orquestrador` â†’ multi-agent                | âš ï¸ ASK FIRST |
| **Complex Task**    | Multiple domains detected                  | `orquestrador` â†’ multi-agent                | âš ï¸ ASK FIRST |
...
function analyzeRequest(userMessage) {
    ...
    if (complexity === "SIMPLE" && domains.length === 1) {
        ...
        return "orquestrador"; // Complex task
    }
}
```
...
```markdown
ðŸ¤– **Applying knowledge of `@auditor-seguranca` + `@backend-specialist`...**
...
| --------------- | ------------------------------------------ | ----------------------- |
| **Security**    | auth, login, jwt, password, hash, token    | `auditor-seguranca`      |
...
| **Performance** | slow, lag, optimize, cache, performance    | `otimizador-desempenho` |
| **SEO**         | seo, meta, analytics, sitemap, robots      | `seo-specialist`        |
| **Game**        | unity, godot, phaser, game, multiplayer    | `game-developer`        |

### Multi-Domain Tasks (Auto-invoke orquestrador)

If request matches **2+ domains from different categories**, automatically use `orquestrador`:

```text
Example: "Create a secure login system with dark mode UI"
â†’ Detected: Security + Frontend
â†’ Auto-invoke: orquestrador
â†’ orquestrador will handle: auditor-seguranca, frontend-specialist, test-engineer
```
...
**Action**: Auto-invoke `orquestrador` â†’ will ask Socratic questions
...
- âŒ Avoid verbose meta-commentary
...
```markdown
ðŸ¤– **Applying knowledge of `@frontend-specialist`...**
...
- **AI detects complex task**: Auto-invoke orquestrador (same result)
...
Expected: Auto-invoke auditor-seguranca
...
Expected: Auto-invoke orquestrador
...
Expected: Auto-invoke depurador
...
```markdown
ðŸ’¡ **Tip**: I am configured with automatic specialist agent selection.
...
âœ… Fallback to orquestrador for complex tasks
