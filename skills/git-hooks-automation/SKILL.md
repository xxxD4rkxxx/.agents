...
Automate code quality enforcement at the Git level. Set up hooks that lint, format, test, and validate before commits and pushes ever reach your CI pipeline â€” catching issues in seconds instead of minutes.
...
Git hooks are scripts that run automatically at specific points in the Git workflow. They live in `.git/hooks/` and are not version-controlled by default â€” which is why tools like Husky exist.
...
# The init command creates a pre-commit hook â€” edit it:
...
project/
â”œâ”€â”€ .husky/
â”‚   â”œâ”€â”€ pre-commit        # npx lint-staged
â”‚   â”œâ”€â”€ commit-msg        # npx --no -- commitlint --edit $1
â”‚   â””â”€â”€ pre-push          # npm test
â”œâ”€â”€ commitlint.config.js
â”œâ”€â”€ package.json          # lint-staged config here
â””â”€â”€ ...
```
...
```bash
#!/bin/sh
# .githooks/pre-commit â€” Team-shared hooks directory
set -e
...
if [ "$BRANCH" = "main" ] || [ "$BRANCH" = "master" ]; then
  echo "âŒ Direct commits to $BRANCH are not allowed. Use a feature branch."
  exit 1
fi

# 2. Check for debugging artifacts
if git diff --cached --diff-filter=ACM | grep -nE '(console\.log|depurador|binding\.pry|import pdb)' > /dev/null 2>&1; then
  echo "âš ï¸  Debug statements found in staged files:"
  git diff --cached --diff-filter=ACM | grep -nE '(console\.log|depurador|binding\.pry|import pdb)'
...
if git diff --cached --diff-filter=ACM | grep -nEi '(AKIA[0-9A-Z]{16}|sk-[a-zA-Z0-9]{48}|ghp_[a-zA-Z0-9]{36}|password\s*=\s*["\x27][^"\x27]+["\x27])' > /dev/null 2>&1; then
  echo "ðŸš¨ Potential secrets detected in staged changes! Review before committing."
  exit 1
fi
...
```json
// âŒ Slow: runs on ALL files every commit
{
  "scripts": {
...
// âœ… Fast: lint-staged runs ONLY on staged files
{
  "lint-staged": {
...
> **Warning**: Bypassing hooks should be rare. If your team frequently bypasses, the hooks are too slow or too strict â€” fix them.

## Migration Guide

### Husky v4 â†’ v9 Migration
...
# 5. Clean up â€” old Husky used package.json config,
...
- **Staged files only** â€” Never lint the entire codebase on every commit
- **Auto-fix when possible** â€” `--fix` flags reduce developer friction
- **Fast hooks** â€” Pre-commit should complete in < 5 seconds
- **Fail loud** â€” Clear error messages with actionable fixes
- **Team-shared** â€” Use Husky or `core.hooksPath` so hooks are version-controlled
- **CI as backup** â€” Hooks are convenience; CI is the enforcer
- **Gradual adoption** â€” Start with formatting, add linting, then testing
