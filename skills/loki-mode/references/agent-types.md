...
Loki Mode has 37 predefined agent types organized into 7 specialized swarms. The orquestrador spawns only the agents needed for your project - a simple app might use 5-10 agents, while a complex startup could spawn 100+ agents working in parallel.
...
1. **Role Switching (Recommended):** orquestrador maintains agent queue, switches roles per task
...
# orquestrador maintains agent queue, switches roles per task
```
...
| Queue depth > 50 | Spawn 5 agents, alert orquestrador | 2min |
