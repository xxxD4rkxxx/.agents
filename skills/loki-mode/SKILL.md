...
3. **CHECK** `.loki/state/orquestrador.json` - Current phase/metrics
...
| - Check orquestrador.json, review pending.json                    |
...
Loki Mode has 37 specialized agent types across 7 swarms. The orquestrador spawns only agents needed for your project.
...
| Can't find what to do | Not following decision tree | Use Decision Tree, check orquestrador.json |
...
+-- state/
|   +-- orquestrador.json   # Master state (phase, metrics)
