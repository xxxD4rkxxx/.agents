...
# Get current phase from orquestrador
CURRENT_PHASE="UNKNOWN"
if [ -f "$LOKI_DIR/state/orquestrador.json" ]; then
    CURRENT_PHASE=$(python3 -c "import json; print(json.load(open('$LOKI_DIR/state/orquestrador.json')).get('currentPhase', 'UNKNOWN'))" 2>/dev/null || echo "UNKNOWN")
fi
