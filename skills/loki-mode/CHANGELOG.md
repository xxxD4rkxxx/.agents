...
|-----|-------------|-------------------------|
| DeepMind | "Hierarchical reasoning separates planning from execution" | orquestrador = planner, agents = executors |
...
|---------|---------------|------------------|
| Multi-turn reasoning | orquestrador-8B | RARV cycle |
...
- **CONSTITUTION.md** - Machine-enforceable behavioral contract (autonomy/CONSTITUTION.md):
  ...
  - Agent behavioral contracts (orquestrador, engineering, QA, DevOps)
...
3. Optional: Enable git checkpoint protocol in orquestrador
...
- **Model Context Protocol (MCP) Integration** - Standardized agent communication:
  ...
  **Architecture**:
  ...
  - orquestrador is MCP client consuming swarm servers
  ...
  **MCP Directory**: `.loki/mcp/` with servers/, orquestrador.ts, registry.yaml
...
- **Autonomous Wrapper Script** (`scripts/loki-wrapper.sh`) - True autonomy with auto-resume:
  ...
  - Completion detection via orquestrador state or `.loki/COMPLETED` marker
